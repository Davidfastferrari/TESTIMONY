...
pub async fn event_handler(provider: Arc<Provider<Ws>>, event_sender: Sender<Event>) {
    ...
    */
    ...
    let router_addresses = vec!["0x80C7DD17B01855a6D2347444a0FCC36136a314de"];
        ...
        match event_receiver.recv().await {
            Ok(event) => match event {
                Event::Block(block) => {
                    ...
                    let eth_usdc_address =
                        Address::from_str("0x57713F7716e0b0F65ec116912F834E49805480d2").unwrap();
                    let pool = pools.get(&eth_usdc_address).unwrap();
                    let reserve = reserves.get(&eth_usdc_address).unwrap();
